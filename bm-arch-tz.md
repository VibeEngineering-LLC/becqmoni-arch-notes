...
Перестроить расчёт вокруг тракта анализа **спектр → счёт → активность → оценка образца → результат**: активность вычисляется в момент расчёта из живых ε (геометрия прибора) и I (набор нуклидов, сменяемый прямо в ходе анализа), доводится до удельной и до заключения по нормативам, а результат сохраняется документом со снапшотом всего использованного. Сущности — Device с геометриями, ROI (зоны и правила, развязанные с нуклидом), NuclideSet — разводятся так, чтобы каждый шаг тракта питался своей живой сущностью, а не замороженной копией.
...
|---|---|---|---|
| А. Зонный = экспресс-превью тракта В | net cps зоны-окна (алгебра примитивов), без поиска пиков, живой при наборе | зона → линия набора (декларация); зоны генерируются конструктором из набора | результат — оценка, уточняется трактом В; окна без нуклида остаются мониторинговым инструментом |
| Б. По выделению | net выделенного участка × 1/(ε·I) единственного идентифицированного пика | пик → линия | сохраняется как есть: панель «Выбранная область» на спектре (счёт, Lc/Ld/Lq, FWHM); строка активности появляется только при выполнении цепочки условий (десять звеньев: шесть if и конъюнкты, EnergySpectrumView.cs:736-762; ключевое `bgTime > 0` — :709) |
