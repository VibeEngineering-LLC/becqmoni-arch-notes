...
- Ключевые поля:
  ...
  - `HasEfficiency` — вычисляемое: `roiEfficiency != null && Count > 1` (:130-136), т.е. **кривой
...
| 9 | DeviceConfigInfo.DeviceType → контроллер прибора | **строка-Id** → `DeviceType.DeviceTypeMap` → `Type` контроллера | `DeviceConfigForm.cs:785` (основной писатель — выбор в форме), конструкторы/миграции (`DeviceConfigInfo.cs:423,506`) | `MeasurementController.CreateDeviceController` :215-260 (`Activator.CreateInstance`), `DeviceConfigForm.cs:432`; ПЛЮС семь мест сравнивают DeviceType со строковыми литералами напрямую, мимо словаря: `DCControlPanel.cs:332`, `DeviceConfigForm.cs:1185, 1295, 1361, 1420, 1524, 1616` — новый тип прибора требует правки всех семи |
...
3. **ε и I**: `K = BecquerelCoefficient` области. Пишется при создании области из нуклида:
   ...
   молча даёт `resultBq = cps·0 = 0` — тихий нуль активности. Кривая из одной точки
   (см. §5.9) — ровно вход в этот путь.
...
Альтернативная ветка «по выделению»: цепочка условий `EnergySpectrumView.cs:703-740` (семь вложенных if, 16 атомарных условий в диапазоне — из них последовательных 6 if / 14, плюс фильтр числа пиков внутри foreach на :730), вычисление :738-762 — ε и I берутся
...
14. **Дублирование между ROI-файлами.** Каждый поставочный файл `config/ROI/*.xml` несёт
    ...
    (по 150 точек ROIEfficiencyData в двух файлах: `Obsidian Marinelli 0.5.xml` и
    `RadiaCode Marinelli 0.5.xml`; у остальных десяти — 0).
